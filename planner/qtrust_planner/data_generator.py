...
def generate_migration_graph(n_assets: int = 100, seed: int = 42, enterprise_topology: bool | None = None) -> Data:
    ...
    for i, (alg, key_size, vendor_pqc_ready, criticality) in enumerate(asset_records):
        # Breaking weak keys first is high priority, but PQC-ready vendors are easy wins
    ...
    # Convert priorities to an order label: highest priority -> 0, lowest -> n-1
    ...
    # Risk label: higher if migrating now would break dependencies or weak key.
    y_risk = np.zeros(n_assets, dtype=np.float32)
    for i, (alg, key_size, vendor_pqc_ready, criticality) in enumerate(asset_records):
        # Migrating something with many dependents is risky
    ...
    # Normalized priority scores (regression target): higher = migrate earlier.
    ...
    data = Data(
        ...
        y_order=torch.tensor(y_order, dtype=torch.long),
        y_risk=torch.tensor(y_risk, dtype=torch.float32),
        y_priority=torch.tensor(y_priority, dtype=torch.float32),
    )
